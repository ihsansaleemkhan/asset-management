	...
	<div class="content-wrapper">
		<div class="record-container">
		  <div class="container-record" style="border-radius: 18px;">
				...
				<div class="row">
				   <form class="filter-form" id="monitorsFilterForm">
					  <div class="row">
					        <div class="col-lg-10">
		              ...
		              <input type="hidden" name="asset_type" value="12" />
					  <input type="hidden" name="action" value="fetch-monitor-list" />
				   </form>
				   
				   <div class="col-md-12" style="padding: 10px 48px;">
						<table id="monitorsTable" class="table table-striped table-bordered" style="width: 100%; text-align: center">
							<thead>
...
$(document).ready(function(){
  ...
  populateMonitorsListTable();
  populateMonitorsListFilterTags();
  
});

$("#monitorsFilterForm").submit( function(event) {
...
$("#monitorsFilterForm").on("click", ".filter-tag-remove", function() {
	let inputName = $(this).parents(".filter-tag").data("name");
	$(this).parents(".filter-tag").remove();
	$("#desktopFilterForm select[name='" + inputName + "'] option:first-child").prop("selected", true);
	populateMonitorsListTable();
 });

function populateMonitorsListFilterTags() {
	let tags = "";
    $.each($("#monitorsFilterForm select"), function(key, value) {
...
function populateMonitorsListTable() {
	
	let table = $('#monitorsTable').dataTable({
		destroy : true,
		scrollX : true,
		responsive : true,
		"searching" : false,
...
function getMonitorsListParam() {
	 let formData = $("#monitorsFilterForm").serializeArray();
	 let jsonFormData = {};
