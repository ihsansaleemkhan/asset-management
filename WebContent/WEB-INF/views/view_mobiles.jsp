				...
				<div class="row">
					<div class="col-md-12 grid-margin stretch-card">
						<div class="title-card position-relative">
							<h5>List of Mobile Phones</h5>
						</div>
					</div>
				</div>

				<div class="row">
				   <form class="filter-form" id="mobilesFilterForm">
					  <div class="row">
					        <div class="col-lg-10">
		              ...
		              <input type="hidden" name="asset_type" value="22" />
					  <input type="hidden" name="action" value="fetch-mobiles-list" />
				   </form>
				   
				   <div class="col-md-12" style="padding: 10px 48px;">
						<table id="mobilesTable" class="table table-striped table-bordered" style="width: 100%; text-align: center">
							<thead>
...
$(document).ready(function(){
  ...
  populateMobilesListTable();
  populateMobilesListFilterTags();
  
});

$("#mobilesFilterForm").submit( function(event) {
...
$("#mobilesFilterForm").on("click", ".filter-tag-remove", function() {
	let inputName = $(this).parents(".filter-tag").data("name");
	$(this).parents(".filter-tag").remove();
	$("#mobilesFilterForm select[name='" + inputName + "'] option:first-child").prop("selected", true);
	populateMobilesListTable();
 });

function populateMobilesListFilterTags() {
	let tags = "";
    $.each($("#mobilesFilterForm select"), function(key, value) {
...
function populateMobilesListTable() {
	
	let table = $('#mobilesTable').dataTable({
		destroy : true,
		scrollX : true,
		responsive : true,
		"searching" : false,
...
function getMobilesListParam() {
	 let formData = $("#mobilesFilterForm").serializeArray();
	 let jsonFormData = {};
