				...
				<div class="row">
					<div class="col-md-12 grid-margin stretch-card">
						<div class="title-card position-relative">
							<h5>List of Servers</h5>
						</div>
					</div>
				</div>

				<div class="row">
				   <form class="filter-form" id="serverFilterForm">
					  <div class="row">
					        <div class="col-lg-10">
		              ...
		              <input type="hidden" name="asset_type" value="8" />
					  <input type="hidden" name="action" value="fetch-servers-list" />
				   </form>
				   
				   <div class="col-md-12" style="padding: 10px 48px;">
						<table id="serversTable" class="table table-striped table-bordered" style="width: 100%; text-align: center">
							<thead>
...
$(document).ready(function(){
  ...
  populateServerListFilterTags();
  
});

$("#serverFilterForm").submit( function(event) {
...
$("#serverFilterForm").on("click", ".filter-tag-remove", function() {
	let inputName = $(this).parents(".filter-tag").data("name");
	$(this).parents(".filter-tag").remove();
	$("#serverFilterForm select[name='" + inputName + "'] option:first-child").prop("selected", true);
	populateServerListTable();
 });

function populateServerListFilterTags() {
	let tags = "";
    $.each($("#serverFilterForm select"), function(key, value) {
...
function populateServerListTable() {
	...
	let table = $('#serversTable').dataTable({
		destroy : true,
		scrollX : true,
		responsive : true,
		"searching" : false,
...
function getServersListParam() {
	 let formData = $("#serverFilterForm").serializeArray();
	 let jsonFormData = {};
