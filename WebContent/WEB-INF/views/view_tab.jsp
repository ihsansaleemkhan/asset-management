	...
	<div class="content-wrapper">
		<div class="record-container">
		  <div class="container-record" style="border-radius: 18px;">
				...
				<div class="row">
				   <form class="filter-form" id="tabFilterForm">
					  <div class="row">
					        <div class="col-lg-10">
		              ...
		              <input type="hidden" name="asset_type" value="18" />
					  <input type="hidden" name="action" value="fetch-tab-list" />
				   </form>
				   
				   <div class="col-md-12" style="padding: 10px 48px;">
						<table id="tabsTable" class="table table-striped table-bordered" style="width: 100%; text-align: center">
							<thead>
...
$(document).ready(function(){
  ...
  populateTabsListTable();
  populateTabsListFilterTags();
  
});

$("#tabFilterForm").submit( function(event) {
...
$("#tabFilterForm").on("click", ".filter-tag-remove", function() {
	let inputName = $(this).parents(".filter-tag").data("name");
	$(this).parents(".filter-tag").remove();
	$("#upsFilterForm select[name='" + inputName + "'] option:first-child").prop("selected", true);
	populateTabsListTable();
 });

function populateTabsListFilterTags() {
	let tags = "";
    $.each($("#tabFilterForm select"), function(key, value) {
...
function populateTabsListTable() {
	
	let table = $('#tabsTable').dataTable({
		destroy : true,
		scrollX : true,
		responsive : true,
		"searching" : false,
...
function getTabsListParam() {
	 let formData = $("#tabFilterForm").serializeArray();
	 let jsonFormData = {};
